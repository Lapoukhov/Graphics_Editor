...
{
    [Serializable]
    class Oct : Shape, ISelectable, IEditable
    {
        public Oct(int X1, int Y1, int X2, int Y2, Pen P)
        {
            this.X1 = X1;
            this.Y1 = Y1;
            this.X2 = X2;
        ...
        public override void Draw(Graphics g)
        {
            ...
            g.DrawLine(P, BX, AY + h, BX, AY + 2 * h);
            g.DrawLine(P, BX, AY + 2 * h, BX - w, BY);
            g.DrawLine(P, BX - w, BY, BX - 2 * w, BY);
            g.DrawLine(P, BX - 2 * w, BY, AX, BY - h);
            g.DrawLine(P, AX, BY - h, AX, AY + h);
        }
    }
}
